...
GET_ALL = True # False gets one chunk (25) True gets all or max (5000)
FULLTEXT = False # Save fulltext
OPENACCESS = True # Search only openaccess documents (so we can get the full text)

query = "(public policy OR impact OR result OR evaluation OR evidence AND climate OR environment)"

if OPENACCESS:
  query = "openaccess(1) AND " + query

doc_srch = ElsSearch(query,'sciencedirect')
doc_srch.execute(ElsClient(client['apikey']), get_all = GET_ALL)
...
for doc in doc_srch.results:
  ...
  if FULLTEXT:
    ...
    if doi_doc.read(ElsClient(client['apikey'])):
        doi_doc.write() 
    else:
